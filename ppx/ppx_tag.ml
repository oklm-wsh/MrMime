...
let make xml =
  let make_tag childs =
    let rec aux acc = function
      | Node (_, tag, childs) :: r
        when is_tag "registry" tag
          && has_tag "record" (tag, childs)
          && has_tag "value" (get_tag "record" (tag, childs)) ->
        let tag = get_leaf @@ get_tag "value" @@ get_tag "record" (tag, childs) in
        aux (Set.add tag acc) r
      | x :: r -> aux acc r
      | [] -> acc
    in
  ...
  match xml with
  | (_, Node (_, tag, childs)) when is_tag "registry" tag -> make_tag childs
  | _ -> Set.empty
...
let make_expr symbol db =
  ...
  in
  [%expr Set.union (Set.of_list [%e (to_expr_lst db)]) [%e symbol]]
...
let cmd =
  let doc = "PPX to fill a Address Literal Tags IANA database" in
  let man =
  [ `S "Description"
  ; `P "$(tname) is a ppx to fill a database from a Address Literal Tags IANA XML file."
  ; `S "Requirement"
  ; `P "The PPX can only refill a: "
  ; `I ("ocaml", "Set.Make(String).t")
  ; `P "In other case, the compilation will fail." ]
  in
  Term.(pure ppx), Term.info "ppx_tag" ~doc ~man

let iana_mapper argv =
  match Term.eval ~argv:(Array.of_list ("ppx_tag" :: argv)) cmd with
  | `Error _   -> exit 1
  | `Ok mapper -> mapper
  | _          -> exit 0

let () = register "tag" iana_mapper
