...
sig
  type result =
    [ `Dirty of string (** In Base64 data,  character  [chr] does not respecting
                           the   predicate   [is_b64]   probably   indicates   a
                           transmission error. In this case, the decoder returns
                           [`Dirty raw]. *)
...
(** Special processing is  performed if fewer than 24 bits  are available at the
    end of the data. A full encoding quantum is always completed at the end of a
    body. When fewer than 24 inputs bits are available in an input group, zero bits
    are added (on the right) to form an integral number of 6-bit groups. Padding at
    the end of the data is performed using the ["="] character.
    ...
    unit of encoded output will be  two characters followed by two ["="] padding
    characters.
    - the  final quantum of encoding  input is exactly 16  bits;  here the final
    unit of encoded output will be three characters followed by on ["="] padding
    character.
...
(** [decode decoder] is:
    - [`Continue] if [decoder] awaits for more input. The client must use {!src}
    to provide it.
    ...
    - [`Error exn] if [decoder] has a padding error ([Wrong_padding] error) - in
    this case, the client doest not trust the content - or a parsing error.
