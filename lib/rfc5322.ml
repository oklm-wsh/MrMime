...
*)
let p_obs_utext p state =
  [%debug Printf.printf "state: p_obs_utext\n%!"];
  ...
  let rec loop decoder = match Uutf.decode decoder with
    | `End -> assert false
    | `Uchar uchar ->
      [%debug Printf.printf "state: p_obs_utext uchar (%d)\n%!" uchar];
    ...
    | `Await ->
      [%debug Printf.printf "state: p_obs_utext (Await)\n%!"];

      cur_chr @ function
      | '\x00' .. '\x7F' as chr ->
        if is_obs_utext chr
        then begin
          [%debug Printf.printf "state: p_obs_utext ascii char (%S)\n%!" (String.make 1 chr)];
          Uutf.Manual.src decoder (String.make 1 chr) 0 1;
          junk_chr @ loop decoder
        end else if Buffer.length buf > 0
        then p (Buffer.contents buf)
        else raise (Error.Error (Error.err_unexpected chr state))
      | uchr ->
        [%debug Printf.printf "state: p_obs_utext part of uchar %d\n%!" (Char.code uchr)];
...
let p_obs_unstruct ?(acc = []) p =
  ...
  let rec loop3 acc =
    ...
    @ fun chr -> match String.length lf, String.length cr, chr with
      ...
      | 0, 0, chr -> loop2 acc
      | 0, n, chr -> loop1 (`CR n :: acc)
      | n, 0, chr -> loop2 (`LF n :: acc)
      | a, b, chr -> loop2 (`CR b :: `LF a :: acc)
...
let p_unstructured p =
  let is_wsp = function '\x09' | '\x20' -> true | _ -> false in
  let is_vchar = function
    | '\x21' .. '\x7e' -> true
    | chr              -> false
  in
  ...
  let rec loop0 acc =
    ...
    @ fun chr -> match has_wsp, has_fws, chr with
      | true, true, ('\x00' .. '\x7F' as chr) when is_vchar chr ->
        p_word @ fun value -> loop0 (value :: `FWS :: `WSP :: acc)
      | true, false, ('\x00' .. '\x7F' as chr) when is_vchar chr ->
        p_word @ fun value -> loop0 (value :: `WSP :: acc)
      | false, false, ('\x00' .. '\x7F' as chr) when is_vchar chr ->
        p_word @ fun value -> loop0 (value :: acc)
      | false, false, ('\x00' .. '\x7F' as chr) when is_wsp chr->
        p_while is_wsp @ fun _ -> p (List.rev (`WSP :: acc))
      (* about the utf-8, [p_obs_unstruct] accepts UTF8-non-ascii, so by
       * extension [p_unstructured] accepts UTF8-non-ascii including ASCII
       * vchar (see above) - and RFC 6532 considers the new
       *
       * vchar = ascii-vchar / UTF8-non-ascii. *)
...
let p_cfws_2digit_cfws p =
  p_cfws
  @ fun _ -> (2 * 2) (function '0' .. '9' -> true | _ -> false)
  @ fun n -> p_cfws
  @ p (int_of_string n)
...
let p_2digit_or_obs p =
  p_try (function '0' .. '9' -> true | _ -> false)
  @ fun n ->
    if n = 2
    then p_while (function '0' .. '9' -> true | _ -> false)
         @ fun n -> p_cfws
         @ p (int_of_string n)
...
let p_obs_year p =
  (* [CFWS] 2*DIGIT [CFWS] *)
  p_cfws
  @ fun _ -> (2 * 0) (function '0' .. '9' -> true | _ -> false)
  @ fun y -> p_cfws
...
let p_year has_already_fws p =
  ...
  p_fws
  @ fun has_wsp has_fws -> p_try (function '0' .. '9' -> true | _ -> false)
  @ fun n ->
    if (has_wsp || has_fws || has_already_fws) && n >= 4
    then p_while (function '0' .. '9' -> true | _ -> false) (* TODO: (4 * 0) is_digit *)
         @ fun y -> p_fws
         @ fun has_wsp has_fws ->
           if has_wsp || has_fws
...
let p_obs_day p =
  p_cfws
  @ fun _ -> (1 * 2) (function '0' .. '9' -> true | _ -> false)
  @ fun d -> p_cfws
...
*)
let p_month p =
  (3 * 3) (function 'a' .. 'z' | 'A' .. 'Z' -> true | _ -> false)
  @ function
    | "Jan" -> p `Jan
    | "Feb" -> p `Feb
    | "Mar" -> p `Mar
