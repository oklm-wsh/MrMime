...
let p_ldh_str p state =
  ...
  then raise (Error.Error (Error.err_unexpected '-' state))
  else p s state
...
let p_general_address_literal p state =
  p_ldh_str (fun tag state ->
             ...
             *)
             ...
             then raise (Error.Error (Error.err_invalid_ipv6 state))
             else raise (Error.Error (Error.err_invalid_tag tag state)))
    state
